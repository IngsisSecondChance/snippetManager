...
public class PrintScriptClient {
    ...
    public PrintScriptClient(RestTemplate restTemplate,
    ...
    public ValidationResultDTO validateSnippet(String code, String version) {
        try {
            ...
            ResponseEntity<ValidationResultDTO> resp = restTemplate.postForEntity(
                validateUrl, request, ValidationResultDTO.class);

            return resp.getBody() != null ? resp.getBody() :
                new ValidationResultDTO(false, "No response", 0, 0);
        } catch (Exception e) {
            return new ValidationResultDTO(false,
                "Error comunicando con PrintScriptManager: " + e.getMessage(), 0, 0);
        }
    }
}
