...
public class TestFormatController {
  ...
  @Autowired private FormatConfigController formatConfigController;
  ...
  void testPutFormatConfig() throws IOException {
    ...
    assertEquals(
        200,
        formatConfigController.putFormatConfig(formatConfigDTO, token).getStatusCode().value());
  ...
  void testGetFormatConfig() {
    ...
    when(configService.getFormatConfig("mockUserId", token))
        .thenReturn(Response.withData(formatConfigDTO));
