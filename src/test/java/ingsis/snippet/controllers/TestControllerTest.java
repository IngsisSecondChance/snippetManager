...
public class TestControllerTest {
    ...
    void testGetTestsForSnippet() {
        ...
        assertEquals(200, testController.getTestsForSnippet("snippetId", token).getStatusCode().value());

        when(testService.getTestsForSnippet("snippetId", token))
                .thenReturn(Response.withError(new Error<>(400, "error")));
        assertEquals(400, testController.getTestsForSnippet("snippetId", token).getStatusCode().value());
    }
    ...
    void testDeleteTest() {
        ...
        when(testService.deleteTest("testId", token)).thenReturn(Response.withError(new Error<>(400, "error")));
