...
public class TestSnippetController {
  ...
  public void testSaveSnippet() {
    ...
    when(snippetService.saveSnippet(snippetDTO, token))
  ...
  void testSaveSnippetFile() {
    MultipartFile file =
        new MockMultipartFile("file", "filename.txt", "text/plain", "println(1)".getBytes());
    ...
    when(snippetService.saveSnippet(
            new SnippetDTO("println(1)", "title", "description", "language", "version"), token))
        .thenReturn(Response.withData(snippetCodeDetails));

    assertEquals(
        snippetCodeDetails,
        snippetController
            .saveSnippetFile(file, "title", "description", "language", "version", token)
            .getBody());
  ...
  @Test
  void testUpdate() {
    UpdateSnippetDTO snippetDTO =
        new UpdateSnippetDTO("code", "id1", "title", "description", "language", "version");
    SnippetCodeDetails snippetCodeDetails =
        new SnippetCodeDetails(
            "mockUsername",
            "mockId",
    ...
    when(snippetService.updateSnippet(snippetDTO, token))
        .thenReturn(Response.withData(snippetCodeDetails));
  ...
  void testUpdateSnippetFile() {
    ...
    when(snippetService.updateSnippet(
            new UpdateSnippetDTO(
                "println(1)", "id1", "title", "description", "language", "version"),
            token))
        .thenReturn(Response.withData(snippetCodeDetails));
    ...
    assertEquals(
        snippetCodeDetails,
        snippetController
            .updateSnippetFile(file, "id1", "title", "description", "language", "version", token)
            .getBody());
  ...
  void testGetSnippetDetails() {
    ...
    when(snippetService.getSnippetDetails("id1", token))
        .thenReturn(Response.withData(snippetCodeDetails));
  ...
  void testShareSnippet() {
    ...
    assertEquals(
        snippetCodeDetails, snippetController.shareSnippet(shareSnippetDTO, token).getBody());

    when(snippetService.shareSnippet(shareSnippetDTO, token))
        .thenReturn(Response.withError(new Error<>(400, "error")));

    assertEquals(
        400, snippetController.shareSnippet(shareSnippetDTO, token).getStatusCode().value());
  }
  ...
  void testGetAccessibleSnippets() {
    ...
    PaginationAndDetails paginationAndDetails =
        new PaginationAndDetails(1, 1, 1, snippetCodeDetailsList);
  ...
  void testGetSnippetUsers() {
    PaginatedUsers users =
        new PaginatedUsers(0, 10, 1, List.of(new User("mockUsername", "mockId")));
