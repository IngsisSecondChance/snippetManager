...
public class ConfigServiceTest {
  ...
  @Autowired private LintUpdateService lintUpdateService;

  @Autowired private FormatUpdateService formatUpdateService;
  ...
  void setUp() {
    ...
    when(permissionsManagerHandler.getAllSnippets(anyString()))
        .thenReturn(
            Response.withData(List.of(snippet.getId(), snippet2.getId(), snippet3.getId())));
  ...
  void testGetFormatConfig() {
    ...
    when(bucketHandler.get("format/mockUserId", token))
        .thenReturn(Response.withData(expectedSerializedConfig));
  ...
  void testGetLintingConfig() {
    ...
    when(bucketHandler.get("lint/mockUserId", token))
        .thenReturn(Response.withData(expectedSerializedConfig));
  ...
  void testExternalServerError() throws IOException {
    ...
    Response<Void> response =
        configService.putLintingConfig(expectedLintingConfig, "mockUserId", token);
    assertTrue(response.isError());
